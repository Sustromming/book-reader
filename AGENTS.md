...
- `pyproject.toml`: project metadata, dependencies, pytest configuration, and uv settings.
- `uv.lock`: locked dependency resolution; keep it in sync with `pyproject.toml`.
- `tests/test_app.py`: parser and `/upload` coverage; add future tests under `tests/` and mirror target modules.

## Build, Test, and Development Commands
- `uv sync`: create/update the locked local environment.
- `uv run python app.py`: run the app locally on `http://127.0.0.1:5000`.
- `uv run python -m py_compile app.py`: fast syntax validation before opening a PR.
- Run `uv run pytest -q` for the automated suite.
...
- Preserve defensive upload validation, stable JSON errors, and server-side logging for unexpected failures.
...
- Minimum manual checks:
  ...
  - files over 100 MB are rejected before parsing
  - theme toggle and scrolling reader behavior still work
  - EPUB-local links, keyboard upload, and print/search behavior still work
